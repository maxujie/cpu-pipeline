...
int main() {
  ...
  for (j = 0; j < n; j++) //对mips数组中每条string输出相应的类型，进入选择RIJ型
  {
    ...
    if (op == "add" || op == "addu" || op == "sub" || op == "subu" ||
        ...
        op == "jr" || op == "jalr" || mips[j] == "NOP")
      s1 = Rinstruction(mips[j], op);
    else if (op == "lui" || op == "addi" || op == "addiu" || op == "andi" ||
             op == "slti" || op == "sltiu" || op == "beq" || op == "bne" ||
             op == "blez" || op == "bgtz" || op == "bltz" || op == "sw" ||
             op == "lw")
...
string Iinstruction(string s0, string op, int line) {
  ...
  int l = op.length();
  int l0 = s0.length();
  if (op == "addi" || op == "addiu" || op == "andi" || op == "slti" ||
      op == "sltiu" || op == "lui" || op == "lw" || op == "sw") {
    if (op == "sw")
      s[0] = 49;
    s[2] = 49;
    if (op == "andi" || op == "lui")
      s[3] = 49;
    if (op == "slti" || op == "sltiu" || op == "lui" || op == "sw")
      s[4] = 49;
    if (op == "addiu" || op == "sltiu" || op == "sw" || op == "lui")
      s[5] = 49;
    ...
    if (op == "lw" || op == "sw") {
      if (s0[8] == '(')
        ...
        j = 1000 * (s0[7] - 48) + 100 * (s0[8] - 48) + 10 * (s0[9] - 48) +
            s0[10] - 48;
      else if (s0[12] == '(')
        j = 10000 * (s0[7] - 48) + 1000 * (s0[8] - 48) + 100 * (s0[9] - 48) +
            10 * (s0[10] - 48) + s0[11] - 48;
      temp2 = Binary(j);
      for (i = 0; i < 16; i++)
        s[16 + i] = temp2[i];
    } else {
      if (s0[l0 - 3] == 44) // l0-3上是逗号imme为一位非负数
        j = s0[l0 - 2] - 48;
      else if (s0[l0 - 4] == 44 && s0[l0 - 3] > 47 &&
               s0[l0 - 3] < 58) // l0-4上是逗号，imme是两位非负数
        j = 10 * (s0[l0 - 3] - 48) + s0[l0 - 2] - 48;
      else if (s0[l0 - 5] == 44 && s0[l0 - 4] > 47 &&
               s0[l0 - 4] < 58) // l0-5是逗号，imme是三位非负数
        j = 100 * (s0[l0 - 4] - 48) + 10 * (s0[l0 - 3] - 48) + s0[l0 - 2] - 48;
      else if (s0[l0 - 6] == 44) // l0-6是逗号，imme是四位非负数
        j = 1000 * (s0[l0 - 5] - 48) + 100 * (s0[l0 - 4] - 48) +
            10 * (s0[l0 - 3] - 48) + s0[l0 - 2] - 48;
      else if (s0[l0 - 7] == 44) // l0-7是逗号，imme是五位非负数
        j = 10000 * (s0[l0 - 6] - 48) + 1000 * (s0[l0 - 5] - 48) +
            100 * (s0[l0 - 4] - 48) + 10 * (s0[l0 - 3] - 48) + s0[l0 - 2] - 48;
      else if (s0[l0 - 4] == 44 &&
               s0[l0 - 3] == '-') // l0-4是逗号，imme是一位负数
        j = (-1) * (s0[l0 - 2] - 48);
      else if (s0[l0 - 5] == 44 &&
               s0[l0 - 4] == '-') // l0-5是逗号，imme是两位负数
  ...
  else if (op == "beq" || op == "bne" || op == "blez" || op == "bgtz" ||
           op == "bltz") {
    if (op != "bltz")
      s[3] = 49;
    if (op == "blez" || op == "bgtz")
      s[4] = 49;
    if (op == "bne" || op == "bgtz" || op == "bltz")
      s[5] = 49;
    temp1 = s0.substr(l + 1, 3);
    temp2 = Regnum(temp1);
    for (i = 0; i < 5; i++)
...
string Rinstruction(string s0, string op) {
  ...
  if (op == "add" || op == "addu") {
    ...
    if (op == "slt" || op == "sltu")
      s[28] = 49;
    if (op == "and" || op == "or" || op == "xor" || op == "nor")
      s[29] = 49;
    if (op == "sub" || op == "subu" || op == "xor" || op == "nor" ||
        op == "slt" || op == "sltu")
      s[30] = 49;
    if (op == "addu" || op == "subu" || op == "or" || op == "nor" ||
        op == "sltu")
      s[31] = 49;
